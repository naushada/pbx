...
> **Status:** ✅ Verbatim copy of the upstream shared-library module (Layer 1) — do not modify locally, then patched with two onprem-pbx additions: `MicroService::dispatch_pbx_routes` (REST URI table) and a `/sip-ws` upgrade gate in `WebConnection::handle_input`. The `HandoffOrdering` test from the TDD plan is deferred to Layer 3 TunnelE2E.

## Onprem-pbx patches

**1. `MicroService::dispatch_pbx_routes(req, db) -> string`** (new method)

Intercepts PBX URI prefixes before the inherited route chain so neither `handle_GET` nor `handle_POST` had to be touched (keeps the inherited regression tests untouched). Routes:
...
| _anything else_ | — | returns empty → caller falls through to the inherited dispatch |

`process_request` calls `dispatch_pbx_routes` first; only if it returns empty does it consult the inherited method-based chain.
...
- `FallsThroughOnInheritedUri` (`/api/v1/account/login` returns empty), `FallsThroughOnUnknownUri`, `FallsThroughOnWrongMethod` (`GET /api/v1/society`) — inherited routes stay reachable.

**2. `/agent` WebSocket upgrade hand-off** (Layer 3)

Mirror of the inherited `/ws/db` upgrade branch. When the on-prem agent dials in to `/agent`, `WebConnection::handle_input`:

1. Detects the WS upgrade (GET `/agent` + `Sec-WebSocket-Key`).
2. Sends the `101 Switching Protocols` response.
3. Hand-off ordering: `remove_handler → m_handle = INVALID`.
...
The inherited `/ws/db` upgrade branch is **unchanged** — three upgrades (`/ws/db`, `/agent`, `/sip-ws`) now coexist on the same `WebConnection::handle_input` path with the same hand-off mechanics.

## HandoffOrdering invariant

All three WS upgrade branches (`/ws/db`, `/sip-ws`, `/agent`) follow the well-known `remove_handler → m_handle=INVALID → publish` ordering invariant:
...
The source-grep approach was chosen over a real-reactor end-to-end test because the bug we're guarding against ("swap two adjacent lines") can pass platform-dependent reactor tests but is unambiguous in the source. The inherited `/ws/db` branch is included as a regression guard against drift in the inherited copy.

The cloud's HTTP/WSS-serving spine. Three classes copied near-verbatim from the upstream shared-library `modules/module/webservice/` and then extended:
...
**Extensions over the upstream copy (Layer 1+):**
...
**Hand-off mechanics.** Identical to the inherited `/ws/db` pattern. Order matters: `reactor()->remove_handler(this, READ_MASK | DONT_CALL)` → `m_handle = ACE_INVALID_HANDLE` → publish raw fd to bridge → `connectionPool().erase(raw)`. If `m_handle` is cleared first, `remove_handler` calls `get_handle()`, gets `-1`, and the fd is never removed from epoll — the reactor then dispatches to a deleted `WebConnection` next time the socket is readable.

## Origin

Files were copied verbatim from the upstream shared-library `modules/module/webservice/` and patched (not re-derived); do not modify the inherited paths locally. Per the project rule documented in [`DESIGN.md §12`](../../../DESIGN.md#12-reuse-map).
