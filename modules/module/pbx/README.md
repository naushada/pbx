...
| `/api/v1/subscriber/login`               | `POST` | `handle_subscriber_login_POST`            | Body `{email, password}`. Strict mode (`PBX_AUTH_STRICT=1`): Mongo lookup by email + bcrypt `verify_password` + `status=="active"`; dev mode synthesises a profile. On success writes a `sessions` row with `expiresAt = now + role_ttl` (resident: 1d, **guard: 7d** — guard kiosks can sit idle longer without re-login) and replies with a `Set-Cookie: session=…; HttpOnly; Secure; SameSite=Strict` header + `{token, subscriber}` body. **The `subscriber` payload is projected to the UI's `Subscriber` shape — `{societyId, flatNumber, displayName, sipUser, role, autoAnswer}`** (see `ui/src/common/app-globals.ts`): `displayName ← name`, `sipUser ← sipUsername`. Same projection-as-strip pattern as `handle_directory_GET` — secrets (`sipHa1`, `portalPasswordHash`) and UI-uninteresting fields (`email`, `flatId`, `status`, `_id`, `phone`) all stay server-side. The rename matters: `SipService.connect` builds `sip:<sipUser>@pbx.local` from this — without the projection, strict-mode login would have produced `sip:undefined@…` and SIP wouldn't register. 401/403 on bad/disabled. |
| `/api/v1/ping`                           | `GET`  | `handle_ping_GET`                         | UI keep-alive (every 30s). Returns `{ok:true, ts}`. **Doubles as the sliding-refresh signal**: best-effort bump of the calling session's `expiresAt = now + role_ttl` (token from `?token=` or `session=` cookie). Best-effort = any DB error is swallowed and the ping always returns 200 (a tunnel hiccup must not drop a UI session); already-expired sessions are not resurrected. |
| `/sip-ws` (pre-upgrade)                  | `GET`  | `handle_sipws_upgrade`                    | Resolves the `?token=` query param (or `session=` cookie) against the `sessions` collection. Returns a `SipWsUpgrade{error, open_meta}`: `error` is a 401 for an absent/unknown/expired session; in **strict mode** it also re-checks the backing `subscribers` row and returns 403 if the subscriber was disabled or removed (the subscriber-lifecycle gate). On success **also slides** `expiresAt` forward by the role TTL (best-effort, like ping), then returns the `{societyId, sipUsername, clientUA}` JSON for the bridge's OPEN frame. |
