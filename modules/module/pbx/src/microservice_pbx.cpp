...
std::string handle_subscriber_import_template_xlsx_GET(const std::string & /*req*/,
                                                         IMongodbClient & /*db*/) {
  // Pre-formatted Excel template with mandatory columns highlighted in
  // yellow. Generated by scripts/generate-subscriber-template.py and
  // shipped at docs/subscribers-template.xlsx via Dockerfile.cloud's
  // `COPY docs/ /opt/pbx-cloud/docs/`. We read it from disk on every
  // request — the file is small (~6 KB) and reading from the local
  // filesystem in the same container is essentially free.
  //
  // Path is relative to the binary's runtime CWD (/opt/pbx-cloud on
  // Heroku, repo root in dev). The Dockerfile sets WORKDIR to a sibling
  // of webgui/ so `../docs/...` matches the same layout the static
  // file handler uses for the UI bundle.
  static const char *kCandidatePaths[] = {
      "../docs/subscribers-template.xlsx",  // production layout (Dockerfile.cloud)
      "docs/subscribers-template.xlsx",     // dev layout (run from repo root)
      nullptr,
  };

  std::string body;
  for (int i = 0; kCandidatePaths[i] != nullptr; ++i) {
    std::ifstream f(kCandidatePaths[i], std::ios::binary);
  ...
  if (body.empty()) {
    return response_error(404, "Not Found",
                          "XLSX template missing — rebuild the cloud image "
                          "(scripts/generate-subscriber-template.py runs at build time)");
  }

  const std::string extra =
      "Content-Disposition: attachment; "
      "filename=subscribers-template.xlsx\r\n";
