...
//
// This file is the single source of truth for the interfaces; the
// Angular web softphone (ui/) re-exports it and layers an Angular
// `InjectionToken` on top, the React Native softphone (mobile/) re-
// exports it as-is. Concrete sip.js wiring is in ./sip-ua-sipjs.ts.
//
...
export interface SipUaOpts {
     ...
     * goes here as a `?token=<bearer>` query param (browsers + RN
     * WebSocket cannot set Authorization on WS upgrades; cookies are
     * the alternative). The query string lives only on the wire, which
     * is TLS — same trust boundary as the REST API.
     */
    wsUrl: string;
