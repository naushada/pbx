...
| `SipFrameDemux` | Reads SipFrames off the cloud tunnel; opens (or reuses) a local TCP socket to Asterisk's `ws://127.0.0.1:8088/ws` for each `stream-id`. Bidirectional bytes pipe. Holds an `m_tunnel` back-pointer for the Asterisk→cloud direction; `on_tunnel_disconnect()` nulls it on a cloud drop, so `CloudConnector::OnConnectedHandler` MUST re-arm it via `set_tunnel(&connector)` on every reconnect (PR #112). Without the re-arm, after the first reconnect every reply (e.g. a REGISTER's `200 OK`) is silently dropped at `if (m_tunnel)` and the browser hangs at "Connecting…". Exact mirror of the cloud-side `SipBridge` re-arm (PR #101). |
| `AsteriskWsFactory` | Per-stream concrete `IAsteriskStream`. Plain TCP + WS upgrade to `chan_pjsip`. |
| `AriClient` | Consumes Asterisk's `/ari/events` JSON. Drives admission (bridge-counter cap at 5), CDR writes, `revoke_subscriber()` for `SUBSCRIBER_REVOKED` frames, `publish_register_snapshot()` for presence reconciliation on (re)connect. **Presence feed** comes from **`ContactStatusChange`** (PR #114) — the PJSIP contact lifecycle event (`Created` on REGISTER, `Removed` on unregister/expiry). `EndpointStateChange` is also handled but only moves when a contact is *qualified*; the agent provisions AORs with `qualify_frequency=0`, so `ContactStatusChange` is the event that actually keeps the directory's `online` flags live. **Conference**: a `StasisStart` whose dialed extension is `conf` is special-cased to `handle_conference_join()` (PR #113) — create-or-attach the society's shared mixing bridge `<society>-conf` (a `409 Conflict` from `create_bridge` means the bridge already exists and is treated as success — PR #115) then `add_channel_to_bridge`. The ARI WS upgrade uses `subscribeAll=true` (PR #104/#105) so endpoint + contact + dialplan-Stasis events all arrive. |
...
| `CallRouter` | Forked-ring driver for **1-to-1 / flat calls only**. Dialed extension → list of `sipUsername` targets (`"0"` → guards via the `subscribers(societyId, role)` index, else flat's active subscribers via the denormalised `flatNumber`). Fans out an `originate` per target; first-answer-wins bridges + tears down losers. The `conf` extension does NOT pass through here — `AriClient::handle_stasis_start` intercepts it before delegating to the router. |
...
| `CloudConnector::OnConnectedHandler` (PR #20) | Glue: wired in `main.cpp`, fires on every (re)connect to (1) re-arm `demux.set_tunnel(&connector)` so the Asterisk→cloud reply path survives a reconnect (PR #112 — see `SipFrameDemux` row), (2) send `AGENT_HELLO` (PR #24) for realm bootstrap, (3) call `ari_client.publish_register_snapshot()` to re-sync the cloud's presence cache. |
...
`jest.setup.ts`. The full **23-file / 200-test** suite is reproducible
...
| Heroku H15 idle drop | Should NOT happen — two complementary keep-alives: (1) `AgentStream` and `BrowserStream` both arm a 25 s outer WS-PING (RFC 6455 `0x9`) per-connection — this is what stops Heroku's 55 s edge timeout. (2) `CloudTunnelTickDriver` ticks `CloudTunnelEndpoint::tick()` every 10 s, driving the inner SipFrame-level heartbeat (PING every 10 s, drop after 5 unanswered = 50 s partition-detection window). Window was 15 s originally (PR #106) — too aggressive for live Heroku latency jitter, dropped mid-REGISTER sessions; loosened to 50 s in the follow-up hotfix. Still well under the 55 s outer-WS timeout. See `docs/design/operations/cloud-tunnel-liveness.md`. | — | If it does happen: check both timers fired (cloud logs) |
