...
6. 🔄 Layer 3 — in progress. ✅ `TunnelE2E*` 8/8 (paired in-memory harness). ✅ `BrowserStream*` 8/8 — first concrete ACE binding: `ACE_Event_Handler + BrowserSink`, owns the `/sip-ws` fd post-upgrade, uses `wsframe::{encode,decode}` for the codec, dispatches text/binary frames to `SipBridge::on_browser_data` and pong-replies to pings. Tests use `socketpair()` so `recv` reads from a real fd without a reactor. The Layer 1/2 `/sip-ws` 503 stub is retired — `WebConnection::handle_input` now completes the WS handshake and hands the raw fd to a freshly-constructed `BrowserStream` (xpmile-mechanic ordering: `remove_handler → m_handle=INVALID → register BrowserStream`). Remaining for Layer 3: cloud-side `AgentStream`, agent-side `AceSslTransport`, ARI WS-events client, `HandoffOrdering` end-to-end test.
