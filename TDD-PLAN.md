...
6. 🔄 Layer 3 — in progress. ✅ `TunnelE2E*` 8/8 (paired in-memory harness). ✅ `BrowserStream*` 8/8. ✅ `AgentStream*` 9/9. ✅ `AceSslTransport*` 10/10 — concrete `ITransport` for `CloudConnector`: `ACE_SSL_Context` + `ACE_SSL_SOCK_Connector` (10s timeout) + HTTP WebSocket upgrade with `wsframe::accept_key` validation; post-upgrade WS-frame layer over the SSL stream; pure-logic helpers (request builder + 101-response validator) fully unit-tested. Remaining for Layer 3: Asterisk ARI WS-events client (pushes parsed events into `AriClient::on_event`), `HandoffOrdering` end-to-end test against a real reactor.
