...
6. 🔄 Layer 3 — in progress. ✅ `TunnelE2E*` 8/8 (paired in-memory harness). ✅ `BrowserStream*` 8/8 — `ACE_Event_Handler + BrowserSink` for `/sip-ws`; Layer 1/2 `/sip-ws` 503 stub retired. ✅ `AgentStream*` 9/9 — symmetric `ACE_Event_Handler` for `/agent` with a private `TransportAdapter` implementing `IAgentTransport` (the endpoint holds the adapter via `unique_ptr`; AgentStream nulls its back-pointer at every release point so a destroyed adapter is never dereferenced); Layer 2 `/agent` info-log stub retired. Remaining for Layer 3: agent-side `AceSslTransport` (real outbound mTLS dial for `CloudConnector`), Asterisk ARI WS-events client, `HandoffOrdering` end-to-end test.
