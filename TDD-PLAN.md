...
5. 🔄 Layer 2 — in progress. ✅ `SipFrameDemux*` 14/14 green (agent-side mirror of `SipBridge`; in-memory `FakeAsteriskFactory` for tests; `AsteriskState` side-channel survives the stream's `unique_ptr` destruction). Remaining: `CloudConnector*`, `AriClient*`, and the real `/sip-ws` hand-off swap that retires the 503 stub.
