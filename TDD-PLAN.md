...
| **L5c** Mobile RN | TDD layers M0 → M3.b + sip engine + auth shell + sign-out + incoming-call vibration + guard kiosk auto-answer (DESIGN.md §9) + ring re-arm after end + concurrent-call busy gate + directory tap-to-call + call history tap-to-redial — 200 jest tests across 23 files. Detailed plan in [`docs/design/mobile-app-tdd.md`](./docs/design/mobile-app-tdd.md) | ✅ Through M3.b (App-shell, sign-out, ring-vibration, guard autoAnswer, ring rearm, busy gate, directory, history); M4 Detox / M5 device matrix gated on simulators + real devices |
