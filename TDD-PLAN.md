...
6. 🔄 Layer 3 — in progress. ✅ `TunnelE2E*` 8/8. ✅ `BrowserStream*` 8/8. ✅ `AgentStream*` 9/9. ✅ `AceSslTransport*` 10/10. ✅ `AriWsClient*` 14/14 — plain-TCP WS to Asterisk `/ari/events` with HTTP Basic auth (kept out of the URL so the password doesn't land in webserver logs); RFC 4648 base64 + WS upgrade request builder pure-logic tested; socketpair-driven dispatch tests cover text→on_event, ping→pong, close→disconnect, multi-frame, partial frame. Remaining: `HandoffOrdering` end-to-end test against a real reactor.
