...
5. 🔄 Layer 2 — in progress. ✅ `SipFrameDemux*` 14/14 green. ✅ `CloudConnector*` 11/11 green (concrete `AceSslTransport` deferred to Layer 3). ✅ `AriClient*` 11/11 green (event-driven; `FakeAriRest` records subscribe + `continue` calls; conference detection by bridge participant count ≥ 3; case-insensitive `cause_txt` normalisation; WS subscription glue deferred to Layer 3). Remaining: the real `/sip-ws` hand-off swap that retires the Layer 1 503 stub (needs the cloud-side `CloudTunnelEndpoint`).
