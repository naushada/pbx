...
| **Inbound-call adopt + hangup** | ✅ landed PR #169 — closed a critical UX hole: after an inbound call was accepted, the user had no UI to hang it up (`IncomingCallOverlay` hid, `SipCallController` didn't know about the call, no `<InCallPanel />` rendered) AND the lingering `'accepted'` `IncomingCall` state latched the busy gate forever, blocking all future inbound rings. New `'adopted'` `CallEvent` jumps idle/post-ended → `'connected'` directly; `SipCallController.adoptInboundCall(handle, peerFlat)` lights the in-call panel for inbound; `SipInboundBridge.onAnswered` callback wires the accepted `SipCallHandle` to it; `IncomingCallController.clear()` + a `createSipEnv` `subscribe` on `callController` drops the lingering inbound state when the call ends. Also narrowed the `state !== 'idle'` re-dial guard in `placeCall` / the reducer's `dial` case to `isCallActive(call)`, so a fresh outbound dial after a previous call ended now works (was a separate pre-existing latch). |
| **Manual reconnect** | ✅ landed PR #170 — `RegistrationContext` extends to `{state, reconnect}`; App.tsx surfaces an idempotent `reconnect` that re-issues `sipUaHandle.start()` on the live env. New `ReconnectButton` renders next to the badge on every post-login screen but ONLY when state is in `{unregistered, terminated, unknown}` — invisible during the connecting / registered states (where the badge already carries the "Connecting…" / "Online" signal). Busy-guard blocks double-press while the previous reconnect is in flight. Mirrors web's Dashboard Connect button (the `SipService.connect()` is similarly idempotent). |
...
— the entire suite (**25 files / 239 tests** + **0 typecheck errors**)
