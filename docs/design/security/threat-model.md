...
| InnerTLS on the `/agent` SIP tunnel | `DESIGN.md` §5 / §10 | ⚠️ Pending (D3) | `/ws/db` already gets InnerTLS-over-WSS via `wsdbagent`. The same layering needs to be added to `AceSslTransport` so the SIP tunnel has the same real-trust property. Until then `/agent` is authenticated-by-obscurity (Heroku terminates outer TLS; agent's cert is not verified end-to-end). |
