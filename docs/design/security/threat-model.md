...
| InnerTLS on the `/agent` SIP tunnel | `DESIGN.md` §6.6 / §7 + [`innertls.md`](./innertls.md) | ✅ PR #19 (merged 2026-05-16) | Shared `WsInnerTlsBridge` (dual-mode `IInnerTlsTransport`) layers `InnerTlsClient` (agent) / `InnerTlsServer` (cloud) over the WS frames after the outer upgrade — same trust boundary as `/ws/db`. Agent: `--inner-tls-{cert,key,ca,hostname}`. Cloud: reuses `--tls-{cert,key,ca}`. `AgentStream` ctor split into `(auto_attach=false)` + `setup_inner_tls()` + `attach()` so buffered outbound frames from a prior disconnect can't race the handshake plaintext-onto-the-wire. |
